...
% data
y1 = squeeze(UNIT.MON.DE_PS.SDF(cont,:,unit));
...
set(gca,'Box','off','TickDir','out','linewidth',2.5,'ylim',[-5 max(y3)+70],'xlim',[0 .250],'FontSize',16)

ylabel('impulses per sec','Fontsize',20);
xlabel('time (s) from stim onset','Fontsize',20);

%xlabel([]); ylabel([]); yticklabels([]);xticklabels([]);

if flag_figsave == 1
    cd(strcat(figDir,'unit\'));
...
%% Unit: SDF - DE vs NDE (w/ error)

unit = 2;

figure;
x = sdftm;
y1 = squeeze(dMUA(unit).monSDF(4,:,1)); % DE | PS
y2 = squeeze(dMUA(unit).monSDF(4,:,3)); % NDE | PS
...
ci1 = ciplot(y1+err1,y1-err1,x,'b',0.1); set(ci1,'linestyle','none','handleVisibility','off');
%errorbar(x,y,err)
plot(x,y2,'color',[0, 0.4470, 0.7410],'linewidth',1); hold on
ci2 = ciplot(y2+err2,y2-err2,x,'b',0.1); set(ci2,'linestyle','none','handleVisibility','off');
set(gca,'box','off','FontSize',14,'linewidth',1.5,'ylim',[0 450],'xlim',[-.100 .500])
xlabel('time (s) from stim onset'); ylabel('impulses per sec');
legend('DE','NDE','location','northeast')
...
saveas(gcf, strcat('DEvsNDE', '.svg'));

%% Single Unit: SDF - Mon vs BIN (w/ error)
...
x = sdftm;
y1 = squeeze(dMUA(unit).monSDF(4,:,1)); % DE | PS
y2 = squeeze(dMUA(unit).binSDF(4,:,1)); % BIN | PS
err1 = squeeze(dMUA(unit).monSDF_SEM(4,:,1));
err2 = squeeze(dMUA(unit).binSDF_SEM(4,:,1));
U1 = y1+1.96.*err1;
L1 = y1-1.96.*err1;
...
set(gca,'box','off','FontSize',14,'linewidth',1.5,'ylim',[0 450],'xlim',[-.100 .500])
xlabel('time (s) from stim onset'); ylabel('impulses per sec');
legend('Monocular','Binocular','location','northeast')
title({[dMUA(unit).penetration,' | AUTO | ','Depth: ' num2str(dMUA(unit).depth(2,1))]},'Interpreter','none')

cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\single\')
saveas(gcf, strcat('sdf', '.svg'));

%% UNIT: Residual Plot
figure('position',[44.33333333333333,101,384,496]);
clear i
cLevel = 4;
tw = 1;

%plot(squeeze(BIN.dMUA.PS.resp(cLevel,tw,:) - MON.dMUA.DE_PS.resp(cLevel,tw,:)),STIM.depths(:,2),'linewidth',2,'Color','k');
plot(squeeze(MON.dMUA.NDE_PS.resp(cLevel,tw,:)),STIM.depths(:,2),'linewidth',2,'Color','b');
hold on
plot(squeeze(DI.dMUA.PS.resp(2,tw,:)),STIM.depths(:,2),'linewidth',2,'Color','g');
vl = vline(0, 'k'); set(vl,'linewidth',1,'linestyle','--');
grid off
xlim([0 450]);
set(gca,'Box','off','linewidth',1.5,'FontSize',12)
hold off
lgn = legend('Residual','Location','southeast','orientation','vertical');
set(lgn, 'FontSize',8);
xlabel('Impulses per sec'); ylabel('Contact # relative to layer 4/5 boundary');

sgtitle({[dMUA(unit).penetration,' | AUTO | ','Electrodes: ' num2str(length(STIM.depths))]},'Interpreter','none')

cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\single\')
saveas(gcf, strcat('resp_residual', '.svg'));
