...
if ~isempty(exception), rethrow(exception); end
    ...
    function t = trialtime(), t = current_frame * RefreshInterval; end
    ...
    function render_scene(present)
        ...
        if iscell(cam_data) && get(hTag.CamVideo,'value')
            ...
            for m=1:ncam
                ...
                if showLargeVideo
                    if present
                        mglsetproperty(Screen.CurrentPosition,'text',sprintf('Current position: %.0f ms',round(current_frame * RefreshInterval)));
        ...
        if tracer_update
            tracer_update = false;
            Screen.update_tracers(MLConfig);
            mglactivategraphic(Screen.EyeTracer,DAQ.eye_present);    % The tracers of these four trackers need reactivation,
            mglactivategraphic(Screen.Eye2Tracer,DAQ.eye2_present);  % since they are activated only once when the scene starts.
        ...
        switch data(current_trial).Ver
            case 1
                ...
                if DAQ.touch_present
                    ...
                    mglsetorigin(Screen.TouchCursor,EyeCal.deg2pix(touch));
                    mglactivategraphic(Screen.TouchCursor,~isnan(touch(:,1)));
                end
                ...
                if isempty(last_update)
                    ...
                    if isfield(obj,'Scale'), TaskObject.Scale = obj.Scale(last_update,:); end
                                ...
                                if 2==TaskObject.Modality(m)  % if the 'Info' field exists and the stimulus is a movie, either MovieStartFrame or MovieStartTime field must exist.
                                    if framebyframe(m)
                                        ...
                                        if frame < movie_length(m) || mglgetproperty(TaskObject.ID(m),'looping')
                                            mglsetproperty(TaskObject.ID(m),'seek',frame);
                                        else
            ...
            otherwise  % 2, 2.1, 2.2, 3
                ...
                frame_no = ceil(Time / RefreshInterval);
                prev_frame = frame_no <= current_frame;
                ...
                if scene_no~=current_scene || new_playback_position
                    ...
                    if 0==current_scene
                        ...
                        param.SceneStartTime = Time(scene_no);
                        ...
                        if iscell(obj.SceneParam)
                            ...
                            if isfield(s,'Scale'), TaskObject.Scale = s.Scale; end
                ...
                mglsetproperty(Screen.CurrentPosition,'text',sprintf('Current position: %.0f ms, Scene: %d',round(current_frame * RefreshInterval),current_scene));
                last_sample = min(round(current_frame * RefreshInterval / SampleInterval) + 1,max_sample);
                first_sample = min(round(max(0,current_frame-1) * RefreshInterval / SampleInterval) + 1,last_sample);
    ...
    function export_video()
        ...
        try
            ...
            Screen.reposition_tracers(MLConfig);
    ...
    function init_trial(export,reload_webcam)
        ...
        % create TaskObject
        if 1<data(current_trial).Ver && 0<current_scene, scenes{current_scene}.fini(param); end  % fini before recreating TaskObject
        ...
        switch data(current_trial).Ver
            ...
            otherwise  % 2, 2.1, 2.2, 3
                ...
                param.ShowJoyCursor = [false DAQ.joystick_present()];
                param.ShowJoy2Cursor = [false DAQ.joystick2_present()];
                
                Tracker = TrackerAggregate();
                if DAQ.eye_present, eye_ = EyeTracker(MLConfig,TaskObject,EyeCal,2); Tracker.add(eye_); end
                if DAQ.eye2_present, eye2_ = Eye2Tracker(MLConfig,TaskObject,Eye2Cal,2); Tracker.add(eye2_); end
                if DAQ.joystick_present, joy_ = JoyTracker(MLConfig,TaskObject,JoyCal,2); Tracker.add(joy_); end
                if DAQ.joystick2_present, joy2_ = Joy2Tracker(MLConfig,TaskObject,Joy2Cal,2); Tracker.add(joy2_); end
                if DAQ.button_present, button_ = ButtonTracker(MLConfig,TaskObject,EyeCal,2); Tracker.add(button_); end
                if DAQ.touch_present, touch_ = TouchTracker(MLConfig,TaskObject,EyeCal,2); Tracker.add(touch_); end
                if DAQ.mouse_present
                    switch data(current_trial).Ver
                        case {2,2.1}, touch_ = TouchTracker_v1(MLConfig,TaskObject,EyeCal,2); Tracker.add(touch_);
                        otherwise, mouse_ = MouseTracker(MLConfig,TaskObject,EyeCal,2); Tracker.add(mouse_);
                    end
                end
        ...
        for m=1:size(nonvisual,1)
            switch nonvisual(m,3)
                ...
                case 5, nonvisual_id(m,1) = mgladdbitmap(mglimresize(ttl_triggered,scale),device);         nonvisual_id(m,2) = mgladdtext(TaskObject.Label{nonvisual(m,2)},device);
            end
        ...
        if reload_webcam && iscell(cam_data)
            for m=1:DAQ.nWebcam
                ...
                if ~isempty(cam_data{m})
                    ...
                    cam_data{m}.Time = floor(cam_data{m}.Time / RefreshInterval);
                end
            end
        end
    end
    ...
    function o = reconstruct_adapter(adapter,args)
        ...
        for m=1:nadapter
            if ~isempty(args{m}) && (isstruct(args{m}{1}) || isa(args{m}{1},'SceneParam'))  % adapter aggregator
                if 3<data(current_trial).Ver  % args{m}{end} is always a cell because of obj.AdapterID
                    a = find(strcmp(args{m}{end}(:,1),'AdapterID'),1);
                    idx = find(args{m}{end}{a,2}==adapterID,1);
                    if ~isempty(idx), o = adapterList{idx}; o.import(args{m}{end}); continue, end  % The adapter is already created
                end
                ...
                if 3<data(current_trial).Ver, adapterID(end+1) = o.AdapterID; adapterList{end+1} = o; end
            else
                switch adapter{m}
                    ...
                    otherwise
                        try
                            if 3<data(current_trial).Ver
                                ...
                                if ~isempty(idx), o = adapterList{idx}; o.import(args{m}); continue, end  % The adapter is already created
                            end
    ...
    function load_data(filename)
        ...
        search_path.base_path{end+1} = [fileparts(mfilename('fullpath')) filesep];
        d = [fileparts(mfilename('fullpath')) filesep 'mgl' filesep]; if exist(d,'dir'), search_path.base_path{end+1} = d; end
        ...
        MLConfig.DAQ = DAQ; create(DAQ,[],data(1).AnalogData);
        MLConfig.Screen = Screen; create(Screen,MLConfig);
        mglsetcontrolscreenzoom(MLConfig.ControlScreenZoom/100);
        RefreshInterval = Screen.FrameLength;
        SampleInterval = 1000 / MLConfig.AISampleRate;
        if isfield(TrialRecord.TaskInfo,'TrialErrorCodes'), error_type = TrialRecord.TaskInfo.TrialErrorCodes; else, error_type = error_type_str; end
    ...
    function UIcallback(hObject,~)
        ...
        switch obj_tag
            ...
            case 'JoystickCursorImage'
                mglsetcontrolscreenshow(false);
                joynum = get(hTag.JoystickNumber,'value');
                [filename,filepath] = uigetfile({'*.bmp;*.gif;*.jpg;*.jpeg;*.tif;*.tiff;*.png;*.avi;*.mpg;*.mpeg','Image/Movie Files'; '*.*','All Files'},'Select a(n) image/movie file',fileparts(MLConfig.(obj_tag){joynum}));
    ...
    function init()
        ...
        addpath(BaseDirectory,[BaseDirectory 'daqtoolbox'],[BaseDirectory 'mgl'],[BaseDirectory 'ext'], ...
            [BaseDirectory 'ext' filesep 'playback'],[BaseDirectory 'ext' filesep 'SlackMatlab']);
        ...
        hFig = figure;
        set(hFig,'tag','mlplayer','units','pixels','position',fig_pos,'numbertitle','off','name','MonkeyLogic Player','menubar','none','resize','on','color',frame_bgcolor);
    ...
    function on_resize(varargin)
        ...
        set(hTxt.Text(1),'position',[x0 y0-213 271 253]);
        ...
        h0 = fig_pos(4)-292; if h0<0, h0 = 0; end
        ...
        h0 = fig_pos(4)-381; if h0<0, h0 = 0; end
