...
switch EyeOrJoy
    case 1
        ...
        old_tform = MLConfig.EyeTransform{devnum,MLConfig.EyeCalibration(devnum)};
    case 2
        if ~exist('devnum','var'), devnum = MLConfig.JoystickNumber; end
        old_tform = MLConfig.JoystickTransform{devnum,MLConfig.JoystickCalibration(devnum)};
end
...
try
    ...
    if ishandle(hFig), tform = old_tform; exit_code = 1; close(hFig); end
    rethrow(err);
end
% end of the dialog
                    ...
                    if isempty(picked_target) || keystop_changed
                        ...
                        if ~isempty(picked_target)
                            ...
                            picked_target_id = [load_cursor(tform.fiximage,tform.fixshape,tform.fixcolor,tform.fixsize*MLConfig.PixelsPerDegree(1),1) ...
                                mglsetproperty(mgladdcircle([0 1 0],fi(4~=tform.reward,1,tform.windowsize)*MLConfig.PixelsPerDegree(1)*2,10),'zorder',1)];
    ...
    function init()
        ...
        hTag.cancelbutton = uicontrol('parent',hFig,'style','pushbutton','tag','cancelbutton','string','Cancel (ESC)','fontsize',fontsize,'callback',callbackfunc);
        ...
        if DAQ.reward_present
            ...
            mglsetproperty(reward_duration_id,'origin',[250 50]*Screen.DPI_ratio,'fontsize',12);
            mglsetproperty(mgladdtext('+/-: Increase/decrease reward by 10 ms',12),'origin',[250 70]*Screen.DPI_ratio,'fontsize',12);
        end
    ...
    function on_resize(varargin)
        ...
        y0 = y0 - 205;
        set(hTag.matrix,'position',[x0+25 y0 200 200]);

        y0 = y0 - 35;
        set(hTag.savebutton,'position',[x0+13 y0+2 110 24]);
        set(hTag.cancelbutton,'position',[x0+130 y0+2 110 24]);
    ...
    function DlgRewardOptions()
        ...
        try
            ...
            if 1==exit_code2
                tform.RewardFuncArgs.JuiceLine = str2double(get(tag(1),'string'));
    ...
    function update_UI()
        ...
        set(hTag.fixcolor,'enable',enable,'backgroundcolor',tform.fixcolor,'foregroundcolor',1-tform.fixcolor);
        set(hTag.fixsize','enable',enable,'string',num2str(tform.fixsize));
        set(hTag.fixinterval,'string',num2str(tform.fixinterval));
        set(hTag.color4uncalibrated,'backgroundcolor',tform.color4uncalibrated,'foregroundcolor',1-tform.color4uncalibrated);
        set(hTag.color4calibrated,'backgroundcolor',tform.color4calibrated,'foregroundcolor',1-tform.color4calibrated);
    ...
    function closeDlg(varargin)
        ...
        if 1~=exit_code
            button = 'No';
            if isempty(old_tform) || size(tform.moving_point,1)~=size(old_tform.moving_point,1) || size(tform.fixed_point,1)~=size(old_tform.fixed_point,1) ...
                    || any(tform.moving_point(:)~=old_tform.moving_point(:)) || any(tform.fixed_point(:)~=old_tform.fixed_point(:))
                mglsetcontrolscreenshow(false);
            ...
            if strcmp(button,'No'), tform = old_tform; end
        end
        exit_code = -1;
        try
