...
[ontarget,rt] = eyejoytrack('touchtarget',sample,hold_radius, '~touchtarget',sample,hold_radius, sample_time);  % touching outside the window will abort the trial
if ~ontarget(1)
    error_type = 3;
end
...
if 0==error_type
    toggleobject([target distractor], 'eventmarker',40);  % display target & distractor
    chosen_target = eyejoytrack('touchtarget', [target distractor], hold_radius, max_reaction_time);
    if 0==chosen_target
