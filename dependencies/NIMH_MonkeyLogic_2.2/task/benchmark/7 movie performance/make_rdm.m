...
if isfield(TrialRecord,'CurrentConditionInfo') && isfield(TrialRecord.CurrentConditionInfo,'deg')
    direction = TrialRecord.CurrentConditionInfo.deg;
else
    direction = 0;  % degree
end
if isfield(TrialRecord,'CurrentConditionInfo') && isfield(TrialRecord.CurrentConditionInfo,'coh')
...
scr_radius = 2.5 * PixelsPerDegree;
scr_direction = mod(direction + 90,360);
frame_size = round((scr_radius + scr_dot_size) * 2) + 1;
center = round(frame_size / 2) - 1;
...
imdata = zeros(frame_size,frame_size,4,nframe,'uint8');
for m=1:nframe
    ...
    for n=1:4, imdata(:,:,n,m) = frame; end
