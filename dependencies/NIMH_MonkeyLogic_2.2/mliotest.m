...
persistent general_selected general_range highfreq_selected highfreq_range voice_range update_interval
daq_started = false;
...
if ~isempty(err), throw(err); end
    ...
    function run_trial()
        ...
        if DAQ.touch_present
            touch_cursor = NaN(1,MLConfig.Touchscreen.NumTouch);
            for m=1:length(touch_cursor)
                touch_cursor(m) = load_cursor(MLConfig.TouchCursorImage,MLConfig.TouchCursorShape,MLConfig.TouchCursorColor,MLConfig.TouchCursorSize,10);
            end
        end
        ...
        if 0 < nttl
            ...
            for m=1:nttl
                TTL(1,m) = mgladdtext(sprintf('%d',TTLAvailable(m)),12);
        ...
        while looping
            ...
            if DAQ.touch_present
                if ~isempty(DAQ.Touch)
                    ...
                    mglactivategraphic(touch_cursor,~isnan(touch(:,1)));
    ...
    function closeDlg(varargin)
        ...
        looping = false;
        for m=DAQ.stimulation_available, stop(DAQ.Stimulation{m}); ao = DAQ.Stimulation{m}; ao.TriggerType = 'Manual'; end
        for m=DAQ.ttl_available, putvalue(DAQ.TTL{m},0); end
