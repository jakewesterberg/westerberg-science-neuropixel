...
if ishandle(hFig), close(hFig); end
    ...
    function oneshot_sample()
        enable_multishot = 'off';
        [data,onset_time,trigger_len] = trigger(new_threshold,1,5);
    ...
    function figure_fini(trigger_len)
    ...
    function [data,onset_time,trigger_len] = trigger(threshold,prepadding_frame,postpadding_frame)
        ...
        if 0==threshold
            ...
            [vb,scanline] = mglgetrasterstatus;
            timeout = false; loop_timer = tic;
            while vb || scanline<threshold
                [vb,scanline] = mglgetrasterstatus;
                if 1 < toc(loop_timer), timeout = true; break, end
            end
            if timeout, messagebox('Time out!!! Decrease the new threshold on the menu!!!','e'); return, end
        end
        mdqmex(9,2);  % present
        ...
        try
            switch obj_tag
                case 'NewThreshold'
                    ...
                    if 0<=val && val<=Screen.ScanLine(2), new_threshold = val; end
                ...
                case 'MultiShotTest'
                    ...
                    for t=1:num_trigger
                        [data{t},onset_time(t),trigger_len(t)] = trigger(new_threshold,1,4);
                        if ~isempty(data)
                            if rising_edge
                    ...
                    hold off;
                    figure_fini(mean(trigger_len(~isnan(trigger_len))));
    ...
    function init()
        ...
        default_threshold = fi(0==varargout{1},0,Screen.ScanLine(1) + round(0.9*(Screen.ScanLine(2)-Screen.ScanLine(1))));
        old_threshold = fi(0==varargout{1},0,Screen.ScanLine(1) + round(varargout{1}*(Screen.ScanLine(2)-Screen.ScanLine(1))));
    ...
    function closeDlg(~,~)
        if 1==exit_code, varargout{1} = fi(0==new_threshold,0,(new_threshold-Screen.ScanLine(1))/(Screen.ScanLine(2)-Screen.ScanLine(1))); end
        stop(DAQ);
        destroy(Screen);
        closereq;
    end
