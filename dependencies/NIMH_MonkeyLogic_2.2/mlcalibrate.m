...
%   Mar 12, 2017    Written by Jaewon Hwang (jaewon.hwang@nih.gov, jaewon.hwang@hotmail.com)
    ...
    methods (Access = protected)
        function xy = raw_signal(obj,xy,offset)  % Calibration method 1: Raw signal
            m = size(xy,1);
            xy = (xy - repmat(obj.tform{1}.offset + offset,m,1)) * obj.rotation_t;
        end
        function xy = origin_gain(obj,xy,offset)  % Calibration method 2: Origin-Gain
            m = size(xy,1);
            xy = (xy - repmat(obj.tform{2}.origin + offset*obj.tform{2}.rotation_rev_t./obj.tform{2}.gain,m,1)) .* repmat(obj.tform{2}.gain,m,1) * (obj.tform{2}.rotation_t * obj.rotation_t);
        end
        function xy = spatial_transform(obj,xy,offset)  % Calibration method 3: 2-D Spatial Transformation
            m = size(xy,1);
            xy = (obj.tform{3}.forward_fcn(obj.tform{3},xy) - repmat(offset,m,1)) * obj.rotation_t;
        end
    ...
    methods
        function obj = mlcalibrate(sig_type,MLConfig,devnum)
            ...
            switch obj.calmethod
                case 1, obj.calfun = @obj.raw_signal;
                case 2, obj.calfun = @obj.origin_gain;
                case 3, obj.calfun = @obj.spatial_transform;
            end
        ...
        function xy = deg2pix(obj,xy)
            [m,n] = size(xy); n = n/2;
            xy = xy .* repmat(obj.ppd,m,n) + repmat(obj.sshalf,m,n);
        end
        ...
        function xy = pix2deg(obj,xy)
            [m,n] = size(xy); n = n/2;
            xy = (xy - repmat(obj.sshalf,m,n)) ./ repmat(obj.ppd,m,n);
        end
        function xy = subject2deg(obj,xy)
            [m,n] = size(xy); n = n/2;
            xy = (xy - repmat(obj.ssrc(1:2) + obj.sshalf,m,n)) ./ repmat(obj.ppd,m,n);
        end
        function xy = subject2pix(obj,xy)
            [m,n] = size(xy); n = n/2;
            xy = xy - repmat(obj.ssrc(1:2),m,n);
        end
        function xy = control2deg(obj,xy)
            [m,n] = size(xy); n = n/2;
            xy = (xy - repmat(obj.cspos(1:2) + obj.sshalf ./ obj.c2s_ratio,m,n)) .* repmat(obj.c2s_ratio ./ obj.ppd,m,n);
        end
        function xy = control2pix(obj,xy)
            [m,n] = size(xy); n = n/2;
            xy = (xy - repmat(obj.cspos(1:2),m,n)) .* repmat(obj.c2s_ratio,m,n);
        end
