...
if ~isempty(err1), rethrow(err1); end
    ...
    function run_scene()
        ...
        while 0==exit_code
            ...
            imdata = getdata(cam);
            if ~isempty(imdata.Frame), image(rgb16to24(imdata.Frame(:,:,end),imdata.Format)); set(hAxis,'XTick',[]); drawnow; end
    ...
    function init()
        ...
        y0 = y0 - 30;
        uicontrol('parent',hDlg,'style','text','position',[x0 y0 90 25],'string','Vertical flip','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','center');
        hTag.VerticalFlip = uicontrol('parent',hDlg,'style','checkbox','position',[x0+100 y0+10 15 15],'tag','VerticalFlip','value',cam.VerticalFlip,'fontsize',fontsize,'callback',callbackfunc);
...
%         if isempty(cam.Roll), set(hTag.Roll,'enable','off'); else, set(hTag.Roll(1),'string',cam.Roll(1)); if isempty(cam.RollRange), set(hTag.Roll(2),'enable','off'); else, set(hTag.Roll(2),'min',cam.RollRange(1),'max',cam.RollRange(2),'value',cam.Roll(1)); end, end
        ...
        y0 = 15;
%         hTag.resetbutton = uicontrol('parent',hDlg,'style','pushbutton','position',[fw-300 y0 90 25],'tag','resetbutton','string','Reset','fontsize',fontsize,'callback',callbackfunc);
