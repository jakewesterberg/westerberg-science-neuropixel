classdef TouchTracker_v1 < mltracker
    ...
    methods
        function obj = TouchTracker_v1(MLConfig,TaskObject,CalFun,DataSource)
        ...
        function acquire(obj,p)
            ...
            if ~isempty(obj.XYData)
                if any(isnan(obj.XYData(end,:)))
                    ...
                    mglactivategraphic(obj.Screen.TouchCursor(1),true);
                    mglsetorigin(obj.Screen.TouchCursor(1),obj.XYData(end,:));
                end
                obj.Success = true;
            else
                obj.Success = false;
