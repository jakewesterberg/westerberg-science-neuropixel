...
try
    ...
    if TrialRecord.TestTrial
        ...
        varargout{1} = runtime(MLConfig,TrialRecord,mltaskobject(taskobject,MLConfig),mltrialdata);
        close(hFig);
        return
    end
    ...
    if TrialRecord.Pause, set(hFig,'resize','on'); pause_menu(); set(hFig,'resize','off'); varargout{1} = MLConfig; end
    ...
    TrialRecord.setDataFile(datafile);
    if alert, alertfunc('task_start',MLConfig,TrialRecord); task_start_alerted = true; end
    TrialRecord.TaskInfo.StartTime = now;
    
    TaskObject = [];
    clip_cursor = ~TrialRecord.SimulationMode && (MLConfig.Touchscreen.On||MLConfig.MouseKey.Mouse) && 1<mglgetadaptercount;
    for trial=1:MLConfig.TotalNumberOfTrialsToRun
        if clip_cursor, mglsetcursorpos(1); else, mglsetcursorpos(-1); end
        ...
        if isuserloopfile(MLConditions)
            ...
            BlockChange = TrialRecord.BlockChange; TrialRecord.new_trial();
        ...
        TrialData.VariableChanges = copyfield(TrialData.VariableChanges,MLEditable,mlsetdiff(fieldnames(MLEditable),'editable'));
        ...
        if TrialRecord.Pause, TrialRecord.InterTrialIntervalTimer = []; set(hFig,'resize','on'); pause_menu(); set(hFig,'resize','off'); varargout{1} = MLConfig; end
        if TrialRecord.Quit || ~looping, break, end
    end
catch exception1
end

try if trial_start_alerted, alertfunc('trial_end',MLConfig,TrialRecord); end, catch, end
try if block_start_alerted, alertfunc('block_end',MLConfig,TrialRecord); end, catch, end
try if task_start_alerted, alertfunc(fi(isempty(exception1),'task_end','task_aborted'),MLConfig,TrialRecord); end, catch, end
...
if exist('fout','var'), close(fout); end
...
if ~isempty(exception2), rethrow(exception2); end
    ...
    function pause_menu()
        ...
        if 0 < DAQ.nIO()
            ...
            if isempty(DAQ.Webcam{1})
                ...
                MenuItem(end+1,:) = [mgladdtext('[I]: I/O test',12) x y 23];
                MenuItem(end+1,:) = [mgladdtext('[W]: Webcam',12) x+150 y 17];
            end
        ...
        while looping
            ...
            if ~isempty(k)
                switch k
                    ...
                    case 23, if 0 < DAQ.nIO(), set_on_move([]); mliotest(MLConfig); set_on_move(); end  % i
                    case 17, if ~isempty(DAQ.Webcam{1}), DlgWebcamSetup(); end  % w
                    case 47, if ~isempty(MLEditable), DlgEditableVariables(); end  % v
                    case 31  % s
    ...
    function reposition_menuitem()
        ...
        ControlScreenRect = mglgetscreeninfo(2,'Rect');
        ControlScreenHalfSize = 0.5 * (ControlScreenRect(3:4) - ControlScreenRect(1:2));
        pos = [0 0.5*(110+30*(nMenuItem-4))];
        mglsetproperty(MenuItem(2:end,1),'origin',repmat(ControlScreenHalfSize - pos,nMenuItem-1,1) + MenuItem(2:end,2:3));
        for item=1:nMenuItem-4, MenuItemRect(item,:) = mglgetproperty(MenuItem(item+4,1),'rect'); end
        if DAQ.isrunning, mglsetproperty(daq_warning,'origin',repmat(ControlScreenHalfSize + pos,2,1) + [0 5; 0 30]); end
    end
            ...
            for m=1:nfield
                ...
                switch MLEditable.editable.(f)
                    ...
                    otherwise
                        ...
                        elseif isnumeric(v)
                            w = 250/numel(v); for n=1:numel(v), x = 265 + (n-1)*w; tag.(f)(n) = uicontrol('parent',hEditable,'style','edit','position',[x cb+3 w-5 22],'string',v(n),'fontsize',fontsize); end
                        else
