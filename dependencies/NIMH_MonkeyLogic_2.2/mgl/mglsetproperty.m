...
try
    while idx < nargin
        ...
        switch method
            ...
            case {'origin','scale','angle','seek'}, mdqmex(6,id,method,double(varargin{idx})); idx = idx+1;  % seek: in seconds
            ...
            otherwise
                valid_id = id(~isnan(id));
                if isempty(valid_id), return, end
                    ...
                    case 'BITMAP'
                        switch method
                            case 'bitmap'
                                ...
                                mdqmex(6,id,method,sz([2 1]),bits);
                        end
                    case 'MOVIE'
                        switch method
                            ...
                            case 'addframe'
                                bits = varargin{idx}; idx = idx+1;
                                if 2==ndims(bits), bits = repmat(bits,[1 1 3]); end
                                dim = ndims(bits);
                                sz = size(bits);
                                if ~any([3 4]==dim) || ~any([1 3 4]==sz(3)), error('The first argument doesn''t look like a bitmap or movie.'); end
                                if ~isa(bits,'uint8')
                                    if max(bits(:))<=1, bits = bits*255; end
                                    bits = cast(bits,'uint8');
                                end
                                if 3==dim, nframe = 1; else nframe = sz(4); end %#ok<*SEPEX>
                                for m=1:nframe
                                    frame = bits(:,:,:,m);
                                    if 1==sz(3), frame = cat(3,255*ones(sz(1:2),'uint8'),repmat(frame,1,1,3)); end
                                    if 3==sz(3), frame = cat(3,255*ones(sz(1:2),'uint8'),frame); end
                                    frame = flipud(reshape(permute(frame,[2 1 3 4]),[],4)');
                                    mdqmex(6,id,method,frame);
                                end
