...
for ML_ = find(ML_Movie)
    ML_Movie_FrameByFrame(ML_) = TaskObject(ML_).MoreInfo.FrameByFrame;
...
ML_IO_Channel = NaN(1,ML_nObject); for ML_=1:ML_nObject, if isfield(TaskObject(ML_).MoreInfo,'Channel'), ML_IO_Channel(ML_) = TaskObject(ML_).MoreInfo.Channel; end, end
...
ML_BenchmarkFrameCount = 0;
    ...
    ML_ObjectStatusRecord.Scale = ones(ML_MaxObjectStatusRecord,ML_nObject);
    ML_ObjectStatusRecord.Angle = zeros(ML_MaxObjectStatusRecord,ML_nObject);
    ML_ObjectStatusRecord.Zorder = zeros(ML_MaxObjectStatusRecord,ML_nObject);
    ...
    function ml_tflip = toggleobject(stimuli, varargin)
        ...
        if 0==ML_RenderingState
            ...
            if ml_new_scene
                ...
                if ~isempty(ml_new_on_TTL)
                    ml_device = [DAQ.TTL{ml_new_on_TTL}]; if ~isempty(ml_device), register(ml_device,'TTL'); end
                    mglactivategraphic(Screen.TTL(:,ml_new_on_TTL),true);
                end
                ml_new_off_TTL = ML_IO_Channel(ml_new_off & ML_TTL);
                if ~isempty(ml_new_off_TTL)
                    ml_device = [DAQ.TTL{ml_new_off_TTL}]; if ~isempty(ml_device), putvalue(ml_device,0); end
            ...
            % render screen
            if ML_scene_updated || ML_ShowJoyCursor(1) || ML_ShowJoy2Cursor(1)
                ML_scene_updated = true;
        ...
        if 3==ML_RenderingState
            ...
            if ml_new_scene && ML_ToggleCount < ML_MaxObjectStatusRecord
                ...
                ML_ObjectStatusRecord.Scale(ML_ToggleCount,:) = TaskObject.Scale;
                ML_ObjectStatusRecord.Angle(ML_ToggleCount,:) = TaskObject.Angle;
                ML_ObjectStatusRecord.Zorder(ML_ToggleCount,:) = TaskObject.Zorder;
                ML_ObjectStatusRecord.BackgroundColor(ML_ToggleCount,:) = Screen.BackgroundColor;
                if any(ml_position_update)
...
%             end
                ...
                switch ML_SampleCycleStage
                    ...
                    case 5
                        ...
                        if ~isempty(ml_touch)
                            ...
                            mglsetorigin(Screen.TouchCursor(1:ml_ntouch),ml_touch);
    ...
    function end_trial()
        ...
        TrialData.ObjectStatusRecord.Angle = ML_ObjectStatusRecord.Angle(1:ML_ToggleCount,:);
        TrialData.ObjectStatusRecord.Zorder = ML_ObjectStatusRecord.Zorder(1:ML_ToggleCount,:);
        ...
        ml_ = find(any(TrialData.ObjectStatusRecord.Status,1));
...
rt = NaN;
    ...
    function warming_up()
        ...
        for ml_=1:10, toggleobject(find(ML_Visual),'status','on'); eyejoytrack('acquirefunc',@dummy_function,[],20); toggleobject(find(ML_Visual),'status','off'); end
        ...
        if ~isempty(ml_g), mgldestroygraphic(ml_g); end
        ...
        if ~isempty(ml_s), mgldestroysound(ml_s); end
